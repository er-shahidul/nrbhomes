...
(function () {
    ...
    var fileTypeExt = {
        ...
        "document": 'doc|pdf|docx|txt|log|msg|pages|rtf|wpd|wps|indd|qxd|xlsx|pps|ppt|pptx|qxp',
    ...
    SFileInput.prototype = {
        ...
        _moveInput: function (element, wrapper_el) {
            ...
            if (self._input) {
                ...
                if (self._settings.multipleFile) {
                    self._createMetaInput(wrapper_el, tempinput);
        ...
        _createRemoveButton: function (el, temp) {
            ...
            remove.innerHTML = '<span style="margin-left: 10px" data-inputCreate="'+self.inputCreated+'" class="btn btn-danger btn-xs remove_button">Remove</span>';

            addEvent(remove, 'click', function () {
                self.fileselected = self.fileselected - 1;
                self.inputCreated = self.inputCreated;
                var cat =  $(this).parent("li").index();
                var inc_cat= parseInt(cat) + 1;
                $('#monogram option:eq(' + inc_cat + ')').remove();
                $('#monogram_update option:eq(' + inc_cat + ')').remove();
                removeNode(temp);
        ...
        _createMetaInput: function (el, temp) {
            var self = this;
            var input = document.createElement("INPUT");
            input.setAttribute("type","text");
            input.setAttribute("name","meta_data[]");
